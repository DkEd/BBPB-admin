...
def get_category(dob_str, race_date_str, mode="10Y"):
    ...
    mode="10Y" -> Senior, V40, V50...
    mode="5Y"  -> Senior, V35, V40, V45...
    """
    try:
        ...
        # --- 5-Year Banding Logic (Locked for Champ) ---
        if mode == "5Y":
            if age < 35: return "Senior"
            if age >= 75: return "V75+"
            base = (age // 5) * 5
            return f"V{base}"
        
        # --- 10-Year Banding Logic (Default for PB Board) ---
        else:
            if age < 40: return "Senior"
            if age >= 70: return "V70+"
            base = (age // 10) * 10
            return f"V{base}"
