...
with col2:
    ...
    r_file = st.file_uploader("Upload PBs CSV (name,distance,time,location,date)", type="csv")
    if r_file and st.button("Process PBs"):
        # Get member info for gender/dob matching
...
if st.button("💾 Backup Database (CSV)"):
    res_df = pd.DataFrame([json.loads(x) for x in r.lrange("race_results", 0, -1)])
    st.download_button("Download PBs", res_df.to_csv(index=False), "pb_backup.csv")
